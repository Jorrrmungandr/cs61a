...
def shifty_shifts(start, goal, limit):
    ...
    def helper(start,goal,limit,diff):
        if diff > limit:
            return 0
        if start == '' or goal == '':
            d = len(start) if start else len(goal)
            return d
        diff += 1 if start[0] != goal[0] else 0
        return helper(start[1:],goal[1:],limit,diff) + (1 if start[0] != goal[0] else 0)
        
    return helper(start,goal,limit,diff)
    # END PROBLEM 6


def meowstake_matches(start, goal, limit):
    """A diff function that computes the edit distance from START to GOAL."""
    # when a mismatch appears, decrement the limit, and if limit < 0, it means we should end the recursion (this method can avoid introducing a newvariable)
    if limit < 0:
        return 0
    if start == '' or goal == '': # Fill in the condition
        ...
        # END
    # when a mismatch appears, we have 3 methods to solve it, we recursively select the best one
    else:
...
def fastest_words(game):
    ...
    for i in words:
        ...
        ret[min_player].append(word_at(game,i))
    return ret
    # END PROBLEM 10
...
def key_distance_diff(start, goal, limit):
    ...
    # BEGIN PROBLEM EC1
    "*** YOUR CODE HERE ***"
    # This method is more accurate, but it's quite slow
    if limit < 0:
        return float('inf')
    elif start == '' or goal == '':
        return abs(len(start) - len(goal))
...
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM EC2
    "*** YOUR CODE HERE ***"
    # enable the diff_func to memorize 
