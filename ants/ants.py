    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                assert isinstance(self, ContainerAnt)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while not (target_place.bees and self.min_range <= distance <= self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        bees = list(ant_place.bees) # get a copy
        Ant.reduce_armor(self, amount)
        for bee in bees:
            Insect.reduce_armor(bee, amount)
            if self.armor <= 0 and bee.place: # if bee has not yet died
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 11
        "*** YOUR CODE HERE ***"
        # we must add first, then we can reduce_armor, for reduce_armor
        # method is called only by deployed insects
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.is_true_queen = QueenAnt.flag
        QueenAnt.flag = False
        # END Problem 13
    ...
    def buff(self, target):
        assert isinstance(target, Ant)
        if not target.is_buffed:
            target.damage *= 2

        if hasattr(target, 'contained_ant') and target.contained_ant:
            self.buff(target.contained_ant)
