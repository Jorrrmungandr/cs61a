...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t) and label(t) == find_value:
        return tree(replace_value, [replace_leaf(branch,find_value,replace_value) for branch in branches(t)])
    else:
...
def multiple_references_explanation():
    return """Because if a variable is repeated, the
lower or upper bound will no longer be corresponded """
...
def quadratic(x, a, b, c):
    ...
    assert a != 0, 'f(t) is not a quadratic!'
    ext = -b/(2*a)
    if ext >= l and ext <= r:
        return interval(min(f(ext),f(l),f(r)),max(f(ext),f(l),f(r)))
    else:
...
def check_par():
    ...
    r2 = interval(1/2,3/2) # Replace this line!
    return r1, r2
